<template>
  <div
    class="bg-[url('/img/bg.jpeg')] bg-cover transition-opacity opacity-90 bg-gray-200 w-full relative overflow-auto h-screen"
  >
    <!-- <video
      autoplay
      muted
      loop
      class="w-screen h-screen fixed left-0 right-0 top-0 transition-opacity opacity-70"
      src="./assets/video/4.mp4 "
    ></video> -->
    <nav
      class="text-white h-20 px-5 fixed top-0 left-0 right-0 z-10 flex items-center justify-between flex-wrap bg-white backdrop-blur-md bg-white/30"
    >
    ...
    <footer class="text-white h-16 z-20 relative flex items-center">
      <div class="container mx-auto flex justify-center">
        <p class="text-center mr-3">
          Copyright © 2022-{{ new Date().getFullYear() }}
          <span class="hover:text-pink-400 underline decoration-2">xiaobai</span>
        </p>
        <p class="text-center hover:text-pink-400 underline decoration-2">
